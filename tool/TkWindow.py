...
class TkWindow(Tk ,metaclass = TkMeta):
    ...
    def __init__(self ,
        ...
        self.error: Callable = showerror
        self.info: Callable = showinfo
    ...
    def start(self) -> NoReturn:
        ...
        self.menu2.add_command(label="关键字" ,command=test)
        self.menu2.add_command(label="关于" ,command=test)
        self.menu2.add_command(label="设置" ,command=test)
        self.menu2.add_command(label="隐藏" ,command=self.withdraw)

        self.menu3.add_command(label="主程序" ,command=test)
...
async def MAIN(*args: tuple ,**kwargs: dict) -> Any:
